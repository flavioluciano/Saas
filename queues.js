...
async function handleVerifyQueue(job) {
    ...
    try {
        ...
        companies.map(async (c) => {
            var companyId = c.id;
            const moveQueue = await Setting_1.default.findOne({
                where: {
                    key: "outsidequeue",
                    companyId
                }
            });
            const moveQueueId = await Setting_1.default.findOne({
                where: {
                    key: "idfila",
                    companyId
                }
            });
            const moveQueueTime = await Setting_1.default.findOne({
                where: {
                    key: "tempofila",
                    companyId
                }
            });
            const idQueue = parseInt(moveQueueId?.value);
            const timeQueue = parseInt(moveQueueTime?.value);
            if (moveQueue?.value === "enabled") {
                if (!isNaN(idQueue) && Number.isInteger(idQueue) && !isNaN(timeQueue) && Number.isInteger(timeQueue)) {
                    ...
                    const tempoAgora = (0, moment_1.default)().utc().format();
                    ...
                    if (count > 0) {
                        tickets.map(async (ticket) => {
                            ...
                            //logger.info(tempoPassado);
                            //logger.info(tempoAgora);
                        });
                    }
                    else {
                    ...
                    //logger.info(idQueue);
                    //logger.info(timeQueue);
                    //console.log(typeof idQueue, idQueue);
                    //console.log(typeof timeQueue, timeQueue);
                }
            }
        });
    }
